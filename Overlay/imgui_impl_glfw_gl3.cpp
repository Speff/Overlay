...
void ImGui_ImplGlfwGL3_RenderDrawLists(ImDrawData* draw_data)
{
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		{
			const ImDrawCmd* pcmd = ImDrawList_GetCmdPtr(cmd_list, cmd_i);
...
bool    ImGui_ImplGlfwGL3_Init(GLFWwindow* window, bool install_callbacks)
{
	...
	io->RenderDrawListsFn = ImGui_ImplGlfwGL3_RenderDrawLists;       // Alternatively you can set this to NULL and call igGetDrawData() after igRender() to get the same ImDrawData pointer.
...
void ImGui_ImplGlfwGL3_NewFrame()
{
	...
	io->DisplayFramebufferScale = { w > 0 ? ((float)display_w / w) : 0, h > 0 ? ((float)display_h / h) : 0 };
	...
	if (glfwGetWindowAttrib(g_Window, GLFW_FOCUSED))
	{
		...
		else
		{
			...
			io->MousePos = { (float)mouse_x, (float)mouse_y };   // Get mouse position in screen coordinates (set to -1,-1 if no mouse / on another screen, etc.)
		}
	}
	else
	{
